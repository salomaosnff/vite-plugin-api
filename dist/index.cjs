...
function operationNameByPathAndMethod(path, method, actions = {
  ...
  const name = path.replace(/\{(.+?)\}/g, (_, p) => "By" + changeCase.pascalCase(p)).replace(/\/+/g, " ");
  return changeCase.camelCase(`${action} ${name}`);
}
class OpenAPIV3Parser {
  ...
  getServiceName(path, method, operation) {
    return this.options.getServiceName?.(path, method, operation) || changeCase.pascalCase(operation.tags?.[0]) || "ApiService";
  }
  ...
  async parse(input) {
    ...
    for (const [path, pathItem] of Object.entries(input.paths)) {
      for (const [method, operation] of Object.entries(pathItem)) {
        const operationId = operation.operationId ?? "\0" + crypto.randomUUID();
...
async function loadApi(api) {
  ...
  if (api.source.startsWith("http")) {
    data = await axios__default.get(api.source).then((res) => res.data);
  } else {
    data = JSON.parse(await promises.readFile(api.source, "utf8"));
  }
...
`;
      ...
      types = await prettier__namespace.format(types, {
        parser: "typescript",
        semi: true,
        singleQuote: true,
        trailingComma: "all",
      ...
      promises.writeFile("src/swagger.d.ts", types);
    },
